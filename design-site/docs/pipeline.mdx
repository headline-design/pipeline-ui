...
WalletConnect is now supported in the standalone Pipeline class at `@pipeline-ui-2/pipeline` and Pipeline Express. It is not yet supported in the full Pipeline UI library but will be soon!
...
<Table>
  ...
  <tbody>
  <tr><td>Pipeline.main</td><td>string "myAlgoWallet"</td><td>"myAlgoWallet" or currently any other string</td><td>Switch between MyAlgo and WalletConnect wallets</td></tr>
  <tr><td>Pipeline.pipeConnector</td><td>true</td><td>Boolean</td><td>Switch between mainNet and testNet</td></tr>
    </tbody>
    </Table>
